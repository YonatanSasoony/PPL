...
import { isAppExp, isBoolExp, isDefineExp, isIfExp, isLetrecExp, isLetExp, isNumExp,
         ...
         Parsed, PrimOp, ProcExp, Program, StrExp, isLetValuesExp, LetValuesExp,CExp ,ValuesBindings,VarDecl } from "./L5-ast";
import { applyTEnv, makeEmptyTEnv, makeExtendTEnv, TEnv } from "./TEnv";
import { isProcTExp, makeBoolTExp, makeNumTExp, makeProcTExp, makeStrTExp, makeVoidTExp,
         parseTE, unparseTExp,
         BoolTExp, NumTExp, StrTExp, TExp, VoidTExp, isTupleTExp, makeEmptyTupleTExp, makeNonEmptyTupleTExp, TupleTExp, isEmptyTupleTExp } from "./TExp";
import { isEmpty, allT, first, rest, cons } from '../shared/list';
import { Result, makeFailure, bind, makeOk, safe3, safe2, zipWithResult, isOk, mapResult } from '../shared/result';
...
const checkEqualType = (te1: TExp, te2: TExp, exp: Exp): Result<true> => //TODO CHECK
...
export const typeofLetValues = (exp: LetValuesExp, tenv: TEnv): Result<TExp> => {
    const tupleArr = mapResult((v : CExp) => typeofExp(v, tenv), map((b : ValuesBindings) => b.val, exp.bindings));

    if ( (!isOk(tupleArr)) || !allT(isTupleTExp,tupleArr.value))
        return makeFailure("invalid types of values in bindings") //: TupleTExp[]
    const VarDeclsArr = map( (b:ValuesBindings)=> b.vars ,exp.bindings);
    const VarDeclsTypes = map( (decls:VarDecl[])=> map((d:VarDecl)=>d.texp ,decls) ,VarDeclsArr);//types[][]
    const VarDeclsNames = map( (decls:VarDecl[])=> map((d:VarDecl)=>d.var ,decls) ,VarDeclsArr);//string[][]
    const flatTypesOfTuplesVals = tupleArr.value.reduce((acc:TExp[] , curr:TupleTExp ) =>
                                             acc.concat(isEmptyTupleTExp(curr)?[]:curr.TEs) ,[]); //types[]
    const flatTypesOfVarDecls = VarDeclsTypes.reduce((acc:TExp[], curr:TExp[])=> acc.concat(curr) , []); //types[]
    const flatNamesOfVarDecls = VarDeclsNames.reduce((acc:string[], curr:string[])=> acc.concat(curr) , []); //string[]
    const constraints = zipWithResult((vardecl, valFromTuple) =>  checkEqualType(vardecl, valFromTuple, exp),
                                                            flatTypesOfVarDecls, flatTypesOfTuplesVals);

    return bind(constraints, _ => typeofExps(exp.body, makeExtendTEnv(flatNamesOfVarDecls, flatTypesOfVarDecls, tenv)));
};
...
export const typeofValues = (rands:CExp[], tenv: TEnv) : Result<TExp> =>
    bind(mapResult((cexp:CExp) => typeofExp(cexp,tenv) , rands),
                (texp:TExp[])=>makeOk((isEmpty(texp)? makeEmptyTupleTExp() : makeNonEmptyTupleTExp(texp)))) ;
...
export const typeofApp = (app: AppExp, tenv: TEnv): Result<TExp> =>
    ...
    (isPrimOp(app.rator) && (app.rator.op === "values"))? typeofValues(app.rands, tenv) : 
...
// console.log("*****************************************************");
// //const x = `(let-values (((a b)(values 1 2))) (+ a b))`;
// const x = `(values ( + 1 2) )`;
// //const x = "(+ 1 2 3)";
// console.log(x);
// const z = L5typeof(x);
// isOk(z)? console.log(z.value) : console.log(z.message);
// console.log("*****************************************************");
// const m = bind(p(x),(se:Sexp) => bind(parseL5Exp(se), (p:Exp)=> typeofExp(p,makeEmptyTEnv())   ) );
// isOk(m)? console.log(m.value) : console.log(m.message);
