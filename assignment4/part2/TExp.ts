...
//new
//we removed the non empty tuple type and instead we used texp,
//therefore we can use tuples everywhere incuding as a return type of a proc or makeing tupels of tupels
...
;; <empty-te>     ::= Empty
;; <tvar>         ::= a symbol starting with T // tvar(id: Symbol, contents; Box(string|boolean))
;; [number -> number * number] number -> tupel
;; number * boolean * boolean    => tupel 
*/
...
// NonEmptyTupleTExp(TEs: NonTupleTExp[]) //new -> nonTuple to TExp
...
const parseCompoundTExp = (texps: Sexp[]): Result<TExp> => {
    ...
    return (pos === -1)  ? bind(parseTupleTExp(texps), (tes:TExp[])=> makeOk(isEmpty(tes)? makeEmptyTupleTExp() : makeNonEmptyTupleTExp(tes))) ://new (tupel case)
...
export const unparseTExp = (te: TExp): Result<string> => {
    ...
    const up = (x?: TExp): Result<string | string[]> =>
        ...
        isTupleTExp(x)? unparseTuple(isNonEmptyTupleTExp(x)? x.TEs : []) ://new
        makeFailure("Never");
