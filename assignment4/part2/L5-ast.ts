...
export type CompoundExp = AppExp | IfExp | ProcExp | LetExp | LitExp | LetrecExp | SetExp | LetValuesExp;//new
export const isCompoundExp = (x: any): x is CompoundExp =>
    isAppExp(x) || isIfExp(x) || isProcExp(x) || isLitExp(x) || isLetExp(x) || isLetrecExp(x) || isSetExp(x) || isLetValuesExp(x);//new
export const expComponents = (e: Exp): CExp[] =>
    ...
    isSetExp(e) ? [e.val] :
    isDefineExp(e) ? [e.val] :
    //isLetValuesExp(e)?  [...e.body, ...map((b) => b.val, e.bindings)] : //new TODO
...
export const makeLetValuesExp = (bindings: ValuesBindings[] ,body: CExp[]) : LetValuesExp => ({tag: "LetValuesExp", bindings:bindings, body: body});
export const isLetValuesExp = (x:any) : x is LetValuesExp => x.tag === "LetValuesExp";
//new
export interface ValuesBindings {tag: "ValuesBindings"; vars: VarDecl[]; val:CExp;}
export const makeValuesBindings = (vars: VarDecl[], val:CExp) : ValuesBindings => ({tag: "ValuesBindings", vars:vars, val:val});
export const isValuesBindings = (x:any) : x is LetValuesExp => x.tag === "ValuesBindings";
...
// export const parseValuesExp = (vals: Sexp[]) : Result<ValuesExp> => //new
//     bind(mapResult(parseL5CExp, vals), (exps:CExp[]) => makeOk(makeValuesExp(exps)));

    // ["let-values",
    // [ [["a","b","c"],["f","0"]], [["x","y","z"],["f","0"]] ],
    // ["+","a","b","c"]]
//new
export const parseVarDeclArr = (decls: Sexp) : Result<VarDecl[]> =>
    ! isArray(decls)? makeFailure("invalid structure") :
    mapResult(parseVarDecl, decls);
//new
export const parseValuesBindings = (bindings: [Sexp,Sexp][]) : Result<ValuesBindings[]> =>
...
//new
export const parseLetValuesExp = (bindings: Sexp, body: Sexp[]): Result<LetValuesExp> => //new
    isEmpty(body)? makeFailure("Body of let-values cannot be empty") :
    ! isGoodBindings(bindings) ? makeFailure("invalid valueBindings") :
    safe2( (bindings: ValuesBindings[], body: CExp[]) => makeOk(makeLetValuesExp(bindings,body)) )
...
const isPrimitiveOp = (x: string): boolean =>
    ["+", "-", "*", "/", ">", "<", "=", "not", "eq?",
     ...
     "number?", "boolean?", "symbol?", "string?", "display", "newline", "values"].includes(x);//new

const isSpecialForm = (x: string): boolean =>
    ["if", "lambda", "let", "quote", "letrec", "set!", "let-values"].includes(x);//new
...
export const unparse = (e: Parsed): Result<string> =>
    ...
    isLetValuesExp(e)? unparseLetValuesExp(e) ://new
    bind(mapResult(unparse, e.exps), (exps: string[]) => makeOk(`(L5 ${exps})`));

//new
const unparseLetValuesExp = (e:LetValuesExp) : Result<string> =>
    safe2((bdgs: string, body: string) => makeOk(`(let-values (${bdgs}) ${body})`))
       (unparseValuesBindings(e.bindings), unparseLExps(e.body));
//new
const unparseValuesBindings = (bindings: ValuesBindings[]): Result<string> =>
    bind(mapResult( (bdg: ValuesBindings) => safe2((vds: string, val: string) => makeOk(`((${vds}) ${val})`))
                                                (unparseVarDeclArr(bdg.vars), unparse(bdg.val)),
        bindings),                                                                   
         (bdgs: string[]) => makeOk(join(" ", bdgs))); 
//new
const unparseVarDeclArr = (decls:VarDecl[]) : Result<string> =>
        bind(mapResult(unparseVarDecl,decls), (str:string[]) => makeOk(join(" ", str)));
...
//const test = bind( p("(let-values (((a b c) (values 1 2 3))) (+ a b c))"), parseL5Exp);
// const test = parseL5(`(L5 (define f
//     (lambda (x)
//     (values 1 1)))
//     (let-values (((a b) (f 0)))
//     (+ a b)))`);
// const test = parseL5(`( L5 (let-values (((n s) (values 1 "str"))) n) )`);
// isOk(test)? console.log(JSON.stringify(test.value)):
...
// {"tag":"VarDecl",
// "var":"f",
// "texp":{"tag":"TVar","var":"T_1","contents":[null]}
// },
// "val":
// {"tag":"ProcExp",
// "args":
...
// "VarDecl",
// "var":"x",
// "texp":{"tag":"TVar","var":"T_2","contents":[null]}
// }],
// "body":
// [{"tag":"ValuesExp",
// "vals":
...
// "NumExp","val":1}]
// }],
// "returnTE":{"tag":"TVar","var":"T_3","contents":[null]}}
// },
// {"tag":"LetValuesExp",
// "vars":[{"tag":"VarDecl","var":"a","texp":{"tag":"TVar","var":"T_4","contents":[null]}},
// {"tag":"VarDecl","var":"b","texp":{"tag":"TVar","var":"T_5","contents":[null]}}],


// "vals":
// {"tag":"ValuesExp",
// "vals":[{"tag":"VarRef","var":"f"},{"tag":"NumExp","val":0}]},
// "body":
// [{"tag":"AppExp","rator":{"tag":"PrimOp","op":"+"},"rands":[{"tag":"VarRef","var":"a"},{"tag":"VarRef","var":"b"}]}]}


// ]}

// let s = p(`(let-values (((a b c) (f 0)))
// (+ a b c))`);
// isOk(s)? console.log(JSON.stringify(s.value)) :
// console.log(s.message);

//let m = parseL5Exp(`(let-values ((a b) (values 1 2)) (+ a b ))`);
// let m = parseL5Exp(`(values 1 2)`);
// isOk(m)? console.log(JSON.stringify(m.value)) :
