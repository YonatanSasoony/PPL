...
import { CExp, Exp, IfExp, LetrecExp, LetExp, ProcExp, Program, SetExp, isCExp, isLetValuesExp, LetValuesExp, ValuesBindings, makeValuesBindings, makeIfExp, parseL5 } from './L5-ast';
...
import { isClosure, makeClosure, Closure, Value, isTupleSExp, TupleSExp } from "./L5-value";
import { isEmpty, first, rest, allT } from '../shared/list';
import { Result, makeOk, makeFailure, mapResult, safe2, bind, isOk, zipWithResult } from "../shared/result";
...
export const applicativeEval = (exp: CExp, env: Env): Result<Value> =>
    ...
    isLetValuesExp(exp)? evalLetValues(exp,env) ://new
    makeFailure(`Bad L5 AST ${exp}`);

//new
const isSameLength = (t: TupleSExp, str:string[]) : Result<boolean> => 
    (t.vals.length === str.length)? makeOk(true) : makeFailure("varDecl[] and tuple does not have the same length");
//new
const evalLetValues = (exp: LetValuesExp, env: Env): Result<Value> => {
    const tupleArr = mapResult((v : CExp) => applicativeEval(v, env), map((b : ValuesBindings) => b.val, exp.bindings));
    if ( (!isOk(tupleArr)) || (! allT(isTupleSExp, tupleArr.value)) )
        return makeFailure("invalid tuples values") //: TupleSExp[]
    const VarDeclsArr = map( (b:ValuesBindings)=> b.vars ,exp.bindings);
    const VarDecls = map( (decls:VarDecl[])=> map((d:VarDecl)=>d.var ,decls) ,VarDeclsArr);//string[][]
    if (!isOk(zipWithResult(isSameLength, tupleArr.value, VarDecls)) || (tupleArr.value.length !== VarDecls.length))
        return makeFailure("invalid bindings ");
    const flatValues = tupleArr.value.reduce((acc:Value[] , curr:TupleSExp )=> acc.concat(curr.vals) ,[]);
    const flatVars = VarDecls.reduce((acc:string[], curr:string[])=> acc.concat(curr) , []);
    return evalSequence(exp.body, makeExtEnv(flatVars, flatValues, env));
}
...
// const test = evalParse(`(let-values (((n s) (values 1 "str"))) n)`);
