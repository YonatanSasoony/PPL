...
import { isEmptyTupleTExp } from './TExp';
...
export const makeTuple = (vals: SExpValue[]) : TupleSExp => ({tag: "TupleSExp", vals:vals});
...
export const valueToString = (val: Value): string =>
    ...
    isEmptySExp(val) ? "'()" :
    isTupleSExp(val)? tupleToString(val) : // new
    isCompoundSExp(val) ? compoundSExpToString(val) :
    "Error: unknown value type "+val 

    //new
export const tupleToString = (t:TupleSExp) : string =>
    isEmptyTupleTExp(t)? `` : t.vals.map((v: SExpValue) => valueToString(v)).join('\n');
