...
import { makeClosure, Value, LazyValue ,makeLazyValue} from './L4-value';
import { makeProcExp,isBoolExp, isNumExp,isStrExp,isPrimOp,isLitExp,isVarRef,isIfExp,isProcExp,isAppExp } from './L4-ast';
...
const applyRecEnv = (env: RecEnv, v: string): Result<CExp> =>
    env.vars.includes(v) ? makeOk(makeProcExp(env.paramss[env.vars.indexOf(v)], env.bodiess[env.vars.indexOf(v)])) :
    applyEnv(env.nextEnv, v);