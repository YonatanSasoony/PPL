...
import { parseL4, makePrimOp, makeAppExp, makeNumExp, makeLetExp, makeBinding, makeVarRef } from './L4-ast';
import { listPrim } from "./evalPrimitive";
import { evalNormalParse, evalNormalProgram } from './L4-normal';
import { isClosure, Value, makeLazyValue } from './L4-value';
import { makeOk, isOk, Result, bind } from "../shared/result";


describe('Part 3 Tests', () => {

    it('test1- evaluates a program which give a side affect in applicative order, but not in normal order',() => {
        expect( bind( parseL4(`
        (L4
        (define foo (lambda (x) (lambda (y) 3)))
        (define x 1)
        (define setF (lambda (y) (set! x 6)))
        (foo (setF 2))
        x)`),evalNormalProgram ) ).to.deep.equal(makeOk(1));
    });
    it('test2-evaluates a program which would give an error in applicative order, but not in normal order',() => {
        expect( bind( parseL4(`
        (L4 
        (define foo (lambda (x) 1)) 
        (define bad (lambda (y) (/ y 0))) 
        (foo (bad 2)) )`),evalNormalProgram ) ).to.deep.equal(makeOk(1));
    });
    it('test3-evaluates a program which would be infinite loop in applicative order, but not in normal order',() => {
        expect(bind(parseL4(`
        (L4 
            (define foo (lambda (x) 1)) 
            (define loop (lambda (y) (loop 4))) 
            (foo (loop 1)) )`), evalNormalProgram)).to.deep.equal(makeOk(1));
    });
});
