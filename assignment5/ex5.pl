...
%.
pick_me_up(Child,Phone) :- parents(Child,Parent1,Parent2), parent_details(Parent1,Phone,true).
pick_me_up(Child,Phone) :- parents(Child,Parent1,Parent2), parent_details(Parent2,Phone,true).

% Signature: active_child(Name)/1
% Purpose: Is a child participates in at least two activities.
%
active_child(Child) :- participate(Child,Act1), participate(Child,Act2), Act1 \= Act2.

% Signature: activity_participants_list(Name, List)/2
% Purpose:  The relationship between an activity name and list of all the children's
%  names that participate at this activity
%
activity_participants_list(Act,L) :- activity(Act,_), findall(Child, participate(Child,Act), L).
...
%
can_register(Child , Act) :- activity(Act,ActDay), 
                             parents(Child, _, _),
                             findall(BusyDay, (participate(Child,A), activity(A,BusyDay)) ,L),
                             not_member(ActDay,L).
