...
(define sqrt2
  (lambda (x init epsilon)
   (car (find-first (sqrt-lzl x init) (lambda (res) (< (cdr res) epsilon))))
  )
)
...
(define get-value$
  (lambda (assoc-list key success fail)
   (cond ((empty? assoc-list) (fail))
         ((equal? (caar assoc-list) key) (success (cdar assoc-list)))
         (else (get-value$ (cdr assoc-list) key success fail))
     )
  )
)

;(get-value$ '((a . 3) (b . 4)) 'b (lambda(x) (* x x )) (lambda()#f))
;(get-value$ '((a . 3) (b . 4)) 'c (lambda(x) (* x x)) (lambda()#f))
...
(define collect-all-values-1
 (lambda (lists key)
   (if (empty? lists) lists
         (letrec ((result (get-value (car lists) key)))
           (cond ((equal? result 'fail) (collect-all-values-1 (cdr lists) key))
                 (else (cons result (collect-all-values-1 (cdr lists) key)) ))))
 )
)
...
;(define l3 '((a . 1) (e . 2)))
;(define l4 '((e . 5) (f . 6)))
;(collect-all-values-1 (list l3 l4) 'e)
;(collect-all-values-1 (list l3 l4) 'k)


(define collect-all-values-2
 (lambda (lists key)
   (if (empty? lists) lists
       (letrec ((result (get-value$ (car lists) key (lambda (x) x) (lambda() 'fail))))
         (cond ((equal? result 'fail) (collect-all-values-1 (cdr lists) key))
                (else (cons result (collect-all-values-2 (cdr lists) key))))))
 )
)

;(collect-all-values-2 (list l3 l4) 'e)
;(collect-all-values-2 (list l3 l4) 'k)
