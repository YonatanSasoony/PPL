...
const parseForExp = (v :Sexp ,params: Sexp[]) :  Result<ForExp> =>
    params.length !== 3 ? makeFailure("Expression not of the form (for <VarDecl> <NumExp> <NumExp> <CExp>)") :
    ! isIdentifier(v) ? makeFailure("First arg of define must be an identifier") :
    bind(mapResult(parseL21CExp, params), 
        (exp: CExp[]) => (isNumExp(exp[0]) && isNumExp(exp[1]) && (exp[0].val <= exp[1].val) )?  makeOk(makeForExp(makeVarDecl(v), exp[0], exp[1], exp[2])) :
